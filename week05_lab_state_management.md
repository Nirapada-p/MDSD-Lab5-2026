...
<img width="1589" height="689" alt="image" src="https://github.com/user-attachments/assets/081cfa0f-a7a7-47c5-8e70-bc99f747a6a8" />

<img width="562" height="331" alt="image" src="https://github.com/user-attachments/assets/afb85ad9-cba3-4f88-8b8c-026a993f581a" />

<img width="651" height="180" alt="image" src="https://github.com/user-attachments/assets/6e7fd276-ff8d-46a6-837d-881ed4a38583" />

<img width="537" height="123" alt="image" src="https://github.com/user-attachments/assets/2ee664b6-3c4d-4582-bacf-94f6a1bcf7ac" />
...
```
เกิดปัญหา ต้องส่ง savedItems และ onSave ผ่านหลายชั้นของ Widget เพื่อให้หน้าที่ถูก Push ไปอีก Route เข้าถึงข้อมูลเดียวกัน ทำให้โค้ด ซับซ้อน ยุ่งยาก
...
<img width="1919" height="766" alt="image" src="https://github.com/user-attachments/assets/fbc8de25-ca35-43b5-a1ba-3727f9231a7e" />

<img width="1919" height="596" alt="image" src="https://github.com/user-attachments/assets/5e434b51-65b0-4e6d-b1a1-1f7e7148196e" />

<img width="1919" height="581" alt="image" src="https://github.com/user-attachments/assets/6c6a8576-587e-4648-a8c5-a204e8204e44" />

<img width="1919" height="482" alt="image" src="https://github.com/user-attachments/assets/0ee592f2-fbee-4070-a017-2980baf898fd" />
...
```
โดยรวม Gemini ก็แนะนำตรงกับกรอบการตัดสินใจในบทเรียน เพราะเลือกเครื่องมือจากลักษณะของ State ว่าเป็น State ที่ใช้เฉพาะจุดหรือเป็น State ที่ต้องแชร์หลายส่วนของแอป ส่วนจุดที่ต่างกัน คือ Gemini เน้นแนะนำ Provider/Riverpod
สำหรับ State ระดับแอป และพูดถึงเรื่อง Global Scope กับการทดสอบค่อนข้างมาก แต่ยังไม่ได้เน้นขอบเขตของ Widget และความจำเป็นในการแชร์ State ให้ชัดเท่ากรอบในบทเรียน
...
```
ฉัน:การเลือก setState, Provider หรือ Riverpod ควรพิจารณาจากขอบเขตของ State อย่างไร

AI:การพิจารณาเลือกใช้ State Management โดยดูจาก "ขอบเขตของ State" (State Scope)
สามารถแบ่งออกเป็น 3 ระดับหลัก ๆ ดังนี้ครับ:
1. Local Scope (ระดับ Widget เดียว)
คือ State ที่ใช้เพื่อควบคุมพฤติกรรมภายใน Widget นั้น ๆ เพียงตัวเดียว เมื่อ
Widget นั้นถูกทำลาย (Dispose) ข้อมูลก็หายไปได้ ไม่ส่งผลต่อส่วนอื่นของแอป
  - เครื่องมือที่ใช้: setState
  - เกณฑ์การพิจารณา:
      - Data stays here: ข้อมูลไม่ต้องส่งต่อไปยังหน้าอื่น หรือ Widget
        ลูกที่ซับซ้อน
      - UI Logic เท่านั้น: เช่น การเปิด/ปิด Checkbox, การเปลี่ยนสีปุ่มชั่วคราว,
        การทำ Animation ในหน้านั้น, หรือการเก็บค่าใน TextField ก่อนกดส่ง
      - Simple & Fast: เน้นความรวดเร็วในการเขียน ไม่ต้องสร้างไฟล์หรือคลาสใหม่
2. Feature / Scoped Scope (ระดับกลุ่มของ Widget หรือหลายหน้าจอ)
คือ State ที่ต้องใช้ร่วมกันระหว่างหลาย ๆ Widget หรือหลายหน้าจอที่อยู่ใน "สายงาน"
เดียวกัน เช่น ข้อมูลในตะกร้าสินค้า หรือข้อมูลสินค้าที่ต้องซิงค์กันระหว่างหน้า
List และ Detail
  - เครื่องมือที่ใช้: Provider หรือ Riverpod
  - เกณฑ์การพิจารณา:
      - Avoid Prop Drilling: เมื่อคุณเริ่มต้องส่งข้อมูลผ่าน Constructor ลงไปหลาย
        ๆ ชั้น (เช่น หน้า A -> หน้า B -> Widget C)
      - Sync Data: เมื่อมีการแก้ไขข้อมูลที่จุดหนึ่ง แล้วต้องการให้ Widget
        อื่นที่ใช้ข้อมูลตัวเดียวกันอัปเดตทันที
      - Lifecycle Awareness: ข้อมูลต้องยังคงอยู่แม้จะสลับหน้าจอไปมา
        (ตราบใดที่ยังอยู่ใน Scope นั้น)
3. Global Scope (ระดับทั้งแอปพลิเคชัน)
คือ State ที่ส่งผลต่อทุกส่วนของแอป ไม่ว่าจะอยู่ที่หน้าไหน หรือลึกแค่ไหนก็ตาม
  - เครื่องมือที่ใช้: Riverpod (แนะนำ) หรือ Provider
  - เกณฑ์การพิจารณา:
      - App-wide Settings: เช่น Theme (Dark/Light), ภาษารองรับ (Locale),
        สถานะการ Login (Auth State)
      - Accessibility: ข้อมูลที่ต้องเรียกใช้ได้จากทุกที่ แม้แต่ในส่วนที่เป็น
        Business Logic ที่ไม่มี BuildContext (ในกรณีของ Riverpod)
ตารางสรุปเปรียบเทียบตามมิติต่าง ๆ

| หัวข้อการพิจารณา         | `setState`                | `Provider`                                       | `Riverpod`                                           |
| :----------------------- | :------------------------ | :----------------------------------------------- | :--------------------------------------------------- |
| **ขอบเขต (Scope)**       | เฉพาะ Widget นั้น (Local) | ตามตำแหน่งใน Widget Tree                         | Global (อยู่นอก Widget Tree)                         |
| **ความซับซ้อน**          | ต่ำมาก                    | ปานกลาง                                          | ปานกลาง - สูง (ในช่วงแรก)                            |
| **การทดสอบ (Testing)**   | ยาก (ต้อง Test ผ่าน UI)   | ทำได้ (แต่ติดเรื่อง Context)                     | **ง่ายมาก** (แยก Logic ออกจาก UI ชัดเจน)             |
| **ความปลอดภัย (Safety)** | สูง (เพราะ scope แคบ)     | เสี่ยงเกิด Runtime Error (ถ้าหา Provider ไม่เจอ) | **สูงมาก** (เช็คได้ตั้งแต่ตอนเขียนโค้ด/Compile-time) |
| **การใช้ Context**       | ไม่จำเป็น                 | **ต้องใช้** `BuildContext`                       | **ไม่จำเป็น** (ใช้ `Ref`)                            |

คำแนะนำในการตัดสินใจ (Rule of Thumb)
1.  ถ้าข้อมูลใช้แค่ใน Widget นี้และลูกของมัน 1-2 ชั้น: ใช้ setState
2.  ถ้าข้อมูลต้องแชร์ข้ามหน้า หรือเป็น Logic ทางธุรกิจ: ใช้ Riverpod หรือ
    Provider
3.  ถ้าต้องการความทันสมัยและลดข้อจำกัดเรื่อง BuildContext: เลือก Riverpod
    ไปเลยครับ เพราะมันถูกออกแบบมาเพื่อแก้ปัญหาที่ Provider เคยมี
    และจัดการขอบเขตของ State ได้ยืดหยุ่นที่สุดในปัจจุบันครับ

ฉัน: ช่วยระบุด้วยว่า Dark Mode, จำนวนคนกดถูกใจ และ Animation หัวใจ แต่ละอย่างมี State อยู่ในขอบเขตระดับใด และเพราะอะไร
AI: จากการวิเคราะห์ฟีเจอร์ที่คุณต้องการ สามารถระบุขอบเขตของ State (State Scope)
ได้ดังนี้ครับ:

1. Dark Mode / Light Mode
  - ระดับขอบเขต: Global Scope (ระดับทั้งแอปพลิเคชัน)
  - เหตุผล:
      - ผลกระทบวงกว้าง: การเปลี่ยนโหมดสีไม่ได้มีผลแค่หน้าใดหน้าหนึ่ง แต่ส่งผลต่อ
        MaterialApp หรือ ThemeData ซึ่งครอบคลุมทุก Widget ในแอป
      - ความคงอยู่ (Persistence):
        ผู้ใช้มักจะตั้งค่านี้ไว้และคาดหวังให้มันมีผลตลอดการใช้งาน
        ไม่ว่าจะเปลี่ยนไปหน้าไหน หรือแม้แต่ปิดแล้วเปิดแอปใหม่ (มักใช้คู่กับ
        Shared Preferences)
      - การเข้าถึง: ทุก Widget จำเป็นต้อง "ฟัง" (Listen)
        ค่านี้เพื่อปรับสีตัวเองให้ถูกต้องตามธีมปัจจุบัน

2. จำนวนคนกดถูกใจ (Like Counter)
  - ระดับขอบเขต: Shared / Feature Scope (ระดับข้ามหน้าจอ/โมดูล)
  - เหตุผล:
      - Single Source of Truth: ข้อมูลตัวเลขนี้ต้องเป็น "ความจริงชุดเดียวกัน"
        ในหลายหน้าจอ (หน้าแสดงรายการสินค้า และ หน้าหน้ารายละเอียดสินค้า)
      - Data Synchronization: หากคุณกดถูกใจที่หน้า "รายละเอียด" (Detail) แล้วกด
        Back กลับมาที่หน้า "รายการ" (List) ตัวเลขต้องเพิ่มขึ้นทันทีโดยไม่ต้อง
        Refresh หน้าจอใหม่
      - Life-cycle: State
        นี้ต้องคงอยู่ตราบเท่าที่ข้อมูลสินค้านั้นยังแสดงผลอยู่บนหน้าจอใดหน้าจอหนึ่งใน
        Navigation Stack

3. Animation หัวใจกระพริบ
  - ระดับขอบเขต: Local Scope (ระดับ Widget เดียว หรือ Ephemeral State)
  - เหตุผล:
      - UI Feedback Only: การกระพริบเป็นเพียงการตอบสนองทางสายตา (Visual
        Feedback) ชั่วคราวหลังจากผู้ใช้กดปุ่ม ไม่ใช่ข้อมูลทางธุรกิจ (Business
        Data)
      - Self-contained: หน้าจออื่นไม่จำเป็นต้องรู้ว่า
        "ตอนนี้หัวใจในหน้าจอนี้กำลังกระพริบอยู่หรือเปล่า" หรือ "Animation
        เล่นไปถึงเฟรมที่เท่าไหร่แล้ว"
      - Short-lived: เมื่อจบ Animation (เช่น 500ms) State
        ของตัวควบคุมการเคลื่อนไหว (AnimationController)
        ก็จะหยุดทำงานและถูกทำลายไปพร้อมกับ Widget นั้น
สรุปเป็นตารางเปรียบเทียบ

| ฟีเจอร์             | ขอบเขต (Scope) | เหตุผลหลัก                 | เครื่องมือที่เหมาะสม        |
| :------------------ | :------------- | :------------------------- | :-------------------------- |
| **Dark Mode**       | **Global**     | มีผลต่อทุกส่วนในแอป        | Riverpod / Provider         |
| **ตัวนับ Like**     | **Shared**     | ต้องซิงค์ข้อมูลระหว่างหน้า | Riverpod / Provider         |
| **Animation หัวใจ** | **Local**      | เป็น UI Logic เฉพาะจุด     | `setState` (StatefulWidget) |

คำแนะนำเพิ่มเติม: หากคุณเลือกใช้ Riverpod สำหรับข้อ 1 และ 2
จะช่วยให้โค้ดสะอาดมาก เพราะ Riverpod ถูกออกแบบมาให้จัดการ Global และ Shared
State ได้อย่างมีประสิทธิภาพโดยไม่ต้องกังวลเรื่อง BuildContext ครับ
...
<img width="1918" height="402" alt="image" src="https://github.com/user-attachments/assets/3e54471d-e03f-4fc4-8825-96d9d3ade254" />
...
<img width="1919" height="498" alt="image" src="https://github.com/user-attachments/assets/53791540-55a9-4292-b09e-1a2ce85674a9" />


## โจทย์ 1: Search Box
```
คำค้นหาสินค้าเลือกใช้ Ephemeral State  เพราะ
1.ใช้จบในหน้าเดียว ตอนนี้แอปมีแค่หน้า HomePage หน้าเดียว คำค้นหาเอาไว้แค่กรองสินค้าที่จะโชว์ในหน้านี้เท่านั้น ไม่ได้มีหน้าอื่นมาขอใช้ค่านี้ด้วย
2.ไม่ได้กระทบภาพรวมแอปมันเป็นแค่ค่าชั่วคราว ไม่ได้ไปยุ่งกับ Business Logic หลักๆ และไม่ต้องคอยส่งต่อให้ใคร 
...
### โจทย์ที่ 2: ปุ่ม "ล้างรายการโปรดทั้งหมด"
```
การเลือก ref.watch / ref.read ใน FavoritesPage:
- ref.watch (ใช้ใน build)  ดึงข้อมูลมาโชว์และสั่ง rebuild หน้าจอทันทีที่รายการเปลี่ยนเพราะเมื่อมีการ เพิ่ม ลบ หรือล้างรายการ หน้าจะได้อัปเดตทันที
- ref.read (ใช้ตอนกดปุ่ม) ใช้ตอนกดปุ่ม เป็นการสั่งงานแค่ครั้งเดียว ไม่จำเป็นต้องคอยติดตามการเปลี่ยนแปลงของ State เหมือน watch
- ปุ่ม ล้างทั้งหมด จะแสดงเฉพาะตอนที่มีรายการโปรดอยู่ favorites.isNotEmpty และก่อนล้างจริงจะมี AlertDialog ให้ยืนยัน เพื่อป้องกันการกดล้างโดยไม่ตั้งใจ
```
